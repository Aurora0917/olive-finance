...
export default function Collateral({
    ...
    const handleConfirm = async () => {
        ...
        try {
            ...
            if (activeTab === "deposit" && onDeposit) {
                await onDeposit(amountValue, selectedToken);
            } else if (activeTab === "withdraw" && onWithdraw) {
    ...
    const unrealizedPnlUSD = unrealizedPnl;
    const currentEquityAfterWithdraw = activeTab === "withdraw" ?
        (newCollateral + unrealizedPnlUSD) :
        (currentCollateral + unrealizedPnlUSD);

    const wouldBeLiquidated = activeTab === "withdraw" && currentEquityAfterWithdraw < currentPositionSize * 0.05;
